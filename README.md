...
> 效果预览（作者本人的实例）：https://famalhaut04.github.io/learning-hub/
...
- **本仓库不包含任何人的 Canvas 凭证。** `canvas_config.json`（含 Token 的真实配置）已被 `.gitignore` 排除，需要你自己创建并填入自己的 Token。
- Token 只保存在你本机，等同于账号凭证——不要提交、不要分享。若怀疑泄露，到 Canvas「账户 → 设置」删除该令牌即立即失效。
- 学习网站仓库里只有页面代码与课程任务摘要（作业名/截止时间/文件名），不含 Token。
